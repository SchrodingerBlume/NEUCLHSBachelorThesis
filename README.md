...
完成了模板结构的重大优化，实现了配置文件的模块化管理，并修复了多个排版细节问题。本版本重点提升了用户体验和模板的可维护性，使配置更加集中化和便捷化。

🔧 **问题修复**

- 修正手动列宽表格 `\sdbiaoge` 的自动扩充功能和表题间距
- 修复附录表格命令异常
- 修正摄氏度单位间距和波浪线符号规范

🚀 **新增功能与改进**

- **模板结构优化**：重构 `main.tex` 结构，所有设置集中至 `settings` 目录
- **配置文件模块化**：新增排版设置、单位设置、超链接设置、脚注设置等独立配置文件
- **参考文献系统改进**：路径调整至 `data/references.bib`，新增跨页控制开关
- **表格功能增强**：`\biaoge` 和 `\sdbiaoge` 新增可选参数"行距"
- **数学排版优化**：改进上下标样式，新增快捷命令 `\shang{}` 和 `\xia{}`
- **字符支持扩展**：支持 Unicode 罗马数字和摄氏度符号
- **排版精度提升**：优化节标题换页和三级标题位置检测
- **格式细节完善**：调整致谢格式，改用 hologo 宏包
