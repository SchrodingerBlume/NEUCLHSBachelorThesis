...
本模板基于TeX Live 2025开发，编译器应选择\XeLaTeX（XeLaTeX），文献工具应选择\textBibTeX，并按照\XeLaTeX $\rightarrow$ \textBibTeX $\rightarrow$ \XeLaTeX $\rightarrow$ \XeLaTeX 的顺序逐次编译。

\subsection{印刷本}

印刷本论文要求双面打印，论文裁切后规格为70 g白色A4打印纸。一律左侧装订。封面用纸由学校统一制作，免费发放。

在\verb|settings/printmode.tex|中设置为双面模式、对称页边距后生成的PDF可以直接用于打印。

\subsection{电子版}

电子版论文应为一个PDF文件（不加密），论文格式与印刷版论文一致。论文学术声明页需手写签名并扫描。

\section{封面}

本部分请进入\verb|settings/cover.tex|进行配置，其中：\verb|\SetCoverTitle{}|用于设置论文总标题，标题会自动断行（如果对断行位置不满意，可以在需要断行处加上\verb|\\|强制换行），此外，在封面标题中使用斜体需要使用\verb|\coverxt{}|命令包围需要斜体的文字，普通的\verb|\textit{}|此处无效；\verb|\SetCoverID{}|用于输入学号；\verb|\SetCoverMajor{}|用于输入专业；\verb|\SetCoverName{}|用于输入姓名；\verb|\SetCoverTeacher{}|用于输入指导教师姓名；\verb|\SetCoverTeacherTitle{}|用于输入指导教师职称；\verb|\SetCoverDate{}|用于输入日期，包括年和月。
注释掉\verb|\usebigcovertrue|则蓝色大封面不会再显示。

\section{学术声明}

本部分请进入\verb|settings/statement.tex|进行配置，命令格式为\verb|\SetUserSignatu|\\
\verb|re{<签署日期>}{<电子签名图片路径>}|，签署日期应包括年月日，电子签名应为\verb|png|格式，从左至右横排签名，长宽比约为3:1，可以选择将\verb|settings/signature.png|文件替换为自己的签名，也可以选择引导至自定义路径。
...
在\verb|settings/other.tex|中可以分别设置目录中的章节的数字编号、章节标题内容中的西文及章节的页码是否使用黑体。此外还可以分别设置正文中各级标题的数字编号与内容中的西文是否使用黑体。
...
添加章节应在\verb|data|中新建\verb|chap0x.tex|，各章节\verb|tex|文件均应当以章节标题命令\verb|\chapter|作为起始，同时需要在\verb|settings/chapter.tex|文件中按照章节顺序使用\verb|\input{data/chap0x}|命令依次插入对应章节。
...
而\verb|\\|用于同一段落中强制换行，即换行后不会首行缩进，如：
\begin{verbatim}
第一段\\第二段
\end{verbatim}
编译得到
...
由此可见，使用\verb|\\|换行后不会产生新的段落，而是会顶格。
...
使用方法与无序列表大致相同，环境名称是\verb|enumerate|。其支持设置编号的格式，由参数控制，格式是\verb|\begin{enumerate}[<可选参数>]|。值得一提的是，当参数缺省时，数字编号与文字的间距是过大的。加上参数\verb|shuzi|再编译即可解决此问题，对比如下：
...
此外，模板还提供了（1）、①、A.和a.四种格式的参数，分别是\verb|kuohao|、\verb|quan|、\verb|daxie|和\verb|xiaoxie|。
...
参考文献的著录应符合国家标准GB/T 7714—2005，所有被引用文献均要列入参考文献中，请将涉及的引文\textBibTeX 信息放入\verb|ref/ref.bib|中。

参考文献一般包括专著、专著中的析出文献、连续出版物、连续出版物的析出文献、专利文献以及电子资源等。参考文献的著录信息一般包括责任者、提名、版本、出版项、页码、获取和访问路径、数字对象唯一标识符等，但不一定全部包括。

\begin{enumerate}[shuzi]
    \item 责任者主要负责创建信息资源（参考文献数据）的人或组织，包括著者、编者、学位论文撰写者、专利申请者或专利权人、报告撰写者、标准提出者、析出文献的著者等，它们往往出现在著录格式的最前面，对应\texttt{ref.bib}文件中的\texttt{author = \{\}}。尽管国标和学校文档中均要求姓氏全大写，但根据学院文档的要求，本模板设置为姓氏仅首字母大写。\texttt{ref.bib}文件中姓名的输入具体原则如下：

    \begin{enumerate}[kuohao]
        \item 人名与人名之间用\texttt{and}隔开，如\texttt{张三 and 李四}；
        \item 西文姓在后，如\texttt{Si Li}；
        \item 若想将西文姓前置，需要在姓后加半角逗号，如\texttt{Zhang, San}；
        \item 名可以缩写，用首字母加点或不加点均可，如\texttt{W. Wang}或\texttt{W Wang}；
        \item 名如果用连字符或空格隔开，如\texttt{Zhao, Lao-Liu}或\texttt{Lao Liu Zhao}，编译后显示Zhao L L，否则如\texttt{Junhao Wu}这种错误写法，只会显示为Wu J；
    ...
    {3}
    {X}
    {\textbf{原文献主要责任者} & \textbf{\textBibTeX 代码} & \textbf{参考文献著录格式}}
    {
    （明）李时珍 & \texttt{李时珍} & 李时珍 \\
    （瑞士）伊迪斯•乔纳斯 & \texttt{乔纳斯} & 乔纳斯 \\
    （美）S.昂温（Stephen Unwin） & \texttt{昂温} & 昂温 \\
    （英）G.昂温（G. Unwin），\newline P. S. 昂温（P. S. Unwin） & \texttt{G. 昂温 and P. S. 昂温} & 昂温 G，昂温 P S \\
    Albert Einstein & \texttt{Albert Einstein} & Einstein A \\
    Amabel Williams-Ellis & \texttt{Amabel Williams-Ellis} & Williams-Ellis A \\
    Augustus De Morgan & \texttt{Augustus \{De Morgan\}} & De Morgan A \\
    Li Jiangning & \texttt{Li, Jiang Ning}或\newline \texttt{Li, Jiang-Ning} & Li J N \\
    }

    \begin{enumerate}[kuohao, resume]
        \item 欧美著者的中译名只著录其姓（同姓不同名的欧美著者，需著录其名的首字母），如表\ref{tab:name}所示；
        \item 著作方式相同的责任者不超过3个时，全部照录。超过3个时，著录前3个责任者。在代码中既可以列出全部作者，也可以在第三个作者之后用\texttt{and others}省略。无论哪种写法，编译器都将自动在其后加“，等”或与之相应的词，如表\ref{tab:omit}所示。
    ...
    {2}
    {X}
    {\textbf{\textBibTeX 代码} & \textbf{参考文献著录格式}}
    {%
    \texttt{钱学森 and 刘再复}	& 钱学森，刘再复\\
    \texttt{李四光 and 华罗庚 and 茅以升} & 李四光，华罗庚，茅以升\\
    \texttt{印森林 and 吴胜和 and 李俊飞 and 冯文杰} & 印森林，吴胜和，李俊飞，等\\
    \texttt{印森林 and 吴胜和 and 李俊飞 and others} & 印森林，吴胜和，李俊飞，等\\
    \texttt{Evenst W. Fordham and Amiad Ali and David A. Turner and John R. Charters} & Fordham E W, Ali A, Turner D A, et al.\\
    }

    \item 题名包括书名、刊名、报纸名、专利题名、报告名、标准名、学位论文名、档案名、舆图名、析出的文献名等，题名后往往使用“[ ]”符号标识文献类型的标识代码和电子资源文献载体的标识代码，如：[M/OL]，标识文献类型是普通图书，文献载体是联机网络，即网上图书。表\ref{tab:atref}列出了文献类型所对应的标识代码及电子资源载体的标识代码与\textBibTeX 中\verb|entrytype|的对应关系。
    ...
    {3}
    {Y}
    {\textbf{类型简称} & \textbf{中文含义} & \textbf{\textBibTeX 中的\texttt{entrytype}}}
    {%
    ...
    基于对biblatex-gb7714-2025宏包的修改，实现了英文参考文献中的标点符号均采用半角；中文参考文献中的标点符号（除括号外）均采用全角\footnote{见刘佩勇老师的注释。}。
...
\begin{enumerate}[shuzi, resume]
    ...
    \begin{enumerate}[quan]
        ...
        \item 文献是学位论文时，著录格式见\parencite{thesis_example1,thesis_example2,thesis_example3}。注意\verb|entrytype|务必使用\verb|@phdthesis|，而不要选择\verb|@mastersthesis|，后者存在bug，或致文献类型标识[D]不显示。
        \item 文献是来自报告时，著录格式见\parencite{report_example1,report_example2,report_example3}。值得一提的是，在文献\parencite{report_example2}一例中，主要责任者是机构而非人名，则需要用一对花括号围起来，防止\textBibTeX 将之缩写。
...
\item 文献来自电子文献时，著录格式为:
    ...
    电子文献载体标识见表\ref{tab:carrier}。在\textBibTeX 中，可以使用\verb|medium|域指定载体标识。
...
论文中引用的文献的标注方法遵照GB/T 7714—2005，采用顺序编码制，即按正文中引用的文献出现的先后顺序连续编码，将序号置于方括号中。例如引用单篇文献时，采用方括号上标的形式置于所引内容最末句的右上角，方法是\verb|\cite{}|，花括号内写入自定义的文献标识码，如：“\verb|……模式\cite{book_example3}|”编译得到“……模式\cite{book_example3}”；同一处引用多篇文献时，应将各篇文献的自定义标识码在花括号内全部列出，各序号间用半角逗号“,”分隔，如：“\verb|……的研究\cite{book_example2,conferenc|\\
\verb|e_example1,thesis_example1}|”编译得到“……的研究\cite{book_example2, conference_example1,collection_example2}”；如遇连续序号，起讫序号间所有文献的自定义标识码都应该填入花括号，如：“\verb|……的研究\cite{confer|\\
\verb|ence_example1,conference_example2,collection_example1,collection_exampl|\\
\verb|e2}|”编译得到“……的研究\cite{conference_example1,conference_example2,collection_example1,collection_example2}”。

正文中如需对引文进行阐述时，应使用\verb|\parencite{}|命令代替\verb|\cite{}|命令，且用法不变，如：“\verb|文献\parencite{book_example1,book_example2,collection_|\\
\verb|example1,collection_example2,thesis_example1,thesis_example2}从不同角度|\\
\verb|阐述了……|”编译得到“文献\parencite{book_example1,book_example2,collection_example1,collection_example2,thesis_example1,thesis_example2}从不同角度阐述了……”。如果希望了解更多 \textBibTeX -gb7714的使用细节，请阅读\verb|参考文件/biblatex-gb7714-2015.pdf|或查看\verb|@huishidong|的GitHub仓库，\href{https://github.com/hushidong/biblatex-gb7714-2015}{\color{blue}{点此跳转}}。
...
论文中某一物理量的名称和符号应统一，公式的物理量变量符号用斜体，物理量常量符号用正体，理应遵循GB/T 3102.11—1993《物理科学和技术中使用的数学符号》：
\begin{enumerate}[shuzi]
  \item 正文中的数学环境应使用\texttt{\$\$}包围，如：“\verb|$A_{italic}$、$B_\text{upright}$、|\\ \verb|$C^{中文也支持}$|”，编译得到“$A_{italic}$、$B_\text{upright}$、$C^{中文也支持}$”。
  ...
  \item 提供波浪线\footnote{\texttt{\textbackslash blx}采用U+FF5E字符，不使用\texttt{\textbackslash textasciitilde}编译得到的“\textasciitilde”。区别在于前者为符合《中华人民共和国国家标准标点符号用法》4.13.1规定的一字宽，而后者非然。}和省略号的快捷命令，如：“\verb|1\blx 2个|”编译得到“1\blx 2个”；“\verb|未完|\\ \verb|待续\slh|”编译得到“未完待续\slh”。
\end{enumerate}
...
本模板引入\verb|siunitx|宏包，一个快速的上手教程如下，更详细的\verb|siunitx|宏包使用细节请阅读\verb|参考文件/siunitex_ZH_CN.pdf|\footnote{感谢赣医一附院神经内科黄旭华的翻译。}。
...
\begin{itemize}
  \item \verb|\unit{\kilogram\metre\per\second}| $\rightarrow$ \unit{\kilogram\metre\per\second}。
...
\begin{itemize}
  ...
  \item \verb|\qty{23.5}{\newton\metre}| $\rightarrow$ \qty{23.5}{\newton\metre}。
  \item \verb|\qty{1200}{\watt\per\square\metre}| $\rightarrow$ \qty{1200}{\watt\per\square\metre}。
...
例如为“\unit{\mg\per\liter}”定义一条快捷命令\verb|\mgl|，需在使用前声明：
\begin{verbatim}
...
推荐使用本模板自定义的表格命令\verb|\biaoge|，其基本框架为：
...
\verb|\biaoge|命令支持自动跨页，且在跨页后会重复表头并加上“续表x.xx”。需强调的是，除非表格总高度已经大于页面总高度，否则应将其后文字部分提前，将表移至次页最前面。
...
G & H & I \\}
\end{verbatim}
其中，列类型可以使用\verb|m|（两端对齐）或\verb|n|（左右居中）。列宽度建议使用\verb|pt|作为单位，如：\verb|n{147pt}n{147pt}n{147pt}|，使用时保证所有\verb|n|中的\verb|pt|数相加和约为441。随着列数与内容的不同，总和会有细微差异（很折磨人），\textbf{请手动精调至表格宽度等于文本宽度（与页眉横线对齐即可），否则续表标题无法居中}。除非自动列宽的\verb|\biaoge|命令的效果实在太差，否则不建议使用此命令。
...
\begin{verbatim}
\sdbiaoge
{表格换行与对齐示例}{tab:makecell}{n{8cm}n{6.7cm}}
{晨起感恩词 & 刘佩勇老师}
{感恩天地滋养万物 & \makecell[l]{我跟别人对齐方式不一样} \\
...
聪明的你一定已经发现，\verb|\makecell|也可用于实现个别单元格采用不同对齐方式。
...
\biaoge
{多行合并示例}
{tab:merge}{Y}{菌株名称 & 基因型}
{\multirow{4}{*}{\textit{DH5$\upalpha$}} & F$^-$ $\upvarphi$80d\textit{lacZ}$\Delta$M15 $\Delta$(\textit{lacZYA-argF}) \\
       & U169 \textit{recA}1 \textit{endA}1 \textit{hsdR}17 \\
       & (rK$^-$ mK$^+$) \textit{phoA} \textit{supE}44 $\uplambda^-$ \textit{thi}-1 \\
       & \textit{gyrA}96 \textit{relA}1 \\
WM3064 & \textit{thrB}1004 \textit{pro} \textit{thi} \textit{rpsL} \textit{hsdS} \\}
\end{verbatim}
编译得到

\biaoge
{多行合并示例}
{tab:merge}{2}{Y}{菌株名称 & 基因型}
{\multirow{4}{*}{\textit{DH5$\upalpha$}} & F$^-$ $\upvarphi$80d\textit{lacZ}$\Delta$M15 $\Delta$(\textit{lacZYA-argF}) \\
       &  U169 \textit{recA}1 \textit{endA}1 \textit{hsdR}17 \\
       & (rK$^-$ mK$^+$) \textit{phoA} \textit{supE}44 $\uplambda^-$ \textit{thi}-1 \\
       & \textit{gyrA}96 \textit{relA}1 \\
...
类似地，可以用\verb|\multicolumn{列数}{*}{内容}|实现列的合并，这里不多赘述。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.999\textwidth]{fig/mice.png}
    \caption{实验小鼠的基本情况}
    \label{fig:mice}
\end{figure}\tuzhu % 如果下一个内容是章节命令，则与其之间不能有空行
\end{verbatim}
编译得到
...
\begin{figure}[H]
    \centering
    \includegraphics[width=13.54cm]{fig/wb.png}
    \caption{XXX基因电泳凝胶图}
    \label{fig:wb}
\end{figure}\tuzhu{注：A. XXX，1. X；2. X；3. X；4. X；5. X；6. X。B. XXX，1. X；2. X；3. X；4. X。}
\end{verbatim}
编译得到
...
当不需要图注时，也应该保留\verb|\tuzhu|（不要带\verb|{}|），如图\ref{fig:mice}的结构那样。必须注意一点：\textbf{\texttt{\textbackslash tuzhu}之后如果紧接的是章节命令（如\texttt{\textbackslash section、\textbackslash subsection}\slh），则两者之间不能有空行}，否则行距会出现异常。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=13.54cm]{fig/wb续图.png}
    \xucaption{fig:wb}
\end{figure}\tuzhu{注：胶图marker标注可仿照此图。}
\end{verbatim}
编译得到
...
用户可以在\verb|settings/other.tex|中设置页末注的计数行为，可选累计计数或每页重新计数，默认为累计计数。
